	...
	$(".jian").click(function() {
		...
		if(quantity == 1) {
			if(confirm("您是否真要删除该条目？")) {
				location = "/shuyuan/CartItemServlet?method=batchDelete&cartItemIds=" + id;
			}
		} else {
...
function batchDelete() {
	...
	location = "/shuyuan/CartServlet?method=batchDelete&cartItemIds=" + cartItemIdArray;
}
...
<c:forEach var="cart" items="${cartlist}">
	...
	<tr align="center">
		<td align="left">
			<input value="${cart.cartItemId }" type="checkbox" name="checkboxBtn" checked="checked"/>
		</td>
		<td align="left" width="70px">
			<a class="linkImage" href="<c:url value='/jsps/book/desc.jsp'/>"><img border="0" width="54" align="top" src="<c:url value='${cart.book.image_b }'/>"/></a>
		</td>
		<td align="left" width="400px">
		    <a href="<c:url value='/jsps/book/desc.jsp'/>"><span>${cart.book.bname }</span></a>
		</td>
		<td><span>&yen;<span class="currPrice">${cart.book.currPrice }</span></span></td>
		<td>
			<a class="jian" id="${cart.cartItemId }Jian"></a><input class="quantity" readonly="readonly" id="${cart.cartItemId }Quantity" type="text" value="${cart.quantity }"/><a class="jia" id="${cart.cartItemId }Jia"></a>
		</td>
		<td width="100px">
			<span class="price_n">&yen;<span class="subTotal" id="${cart.cartItemId }Subtotal">${cart.subtotal }</span></span>
		</td>
		<td>
			<a href="<c:url value='/CartServlet?method=batchDelete&cartItemIds=${cart.cartItemId }'/>">删除</a>
		</td>
	</tr>
</c:forEach>
	...
	<form id="jieSuanForm" action="<c:url value='/CartServlet'/>" method="post">
		...
		<input type="hidden" name="method" value="loadCart"/>
	</form>
